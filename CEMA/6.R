...
# Option 2: Include VisitDate as a random slope (if you believe growth varies over time)
#mixed_model_random <- lmer(ADWG ~ CalfSex + Education + Distance_water + Weight + Age + 
#                             ManualPCV + Theileria.spp. + ELISA_parva + 
#                             Q.Strongyle.eggs + (1 + VisitDate | CalfID), 
#                           data = ideal_data)

# Step 4: Model summary
mixed_model_summary <- summary(mixed_model)
print(mixed_model_summary)
#mixed_model_random_summary <- summary(mixed_model_random)
#print(mixed_model_random_summary)

# Step 5: Model Diagnostics
# Residuals vs Fitted
plot(mixed_model, which = 1)
#plot(mixed_model_random, which = 1)
...
ggplot(data = as.data.frame(residuals(mixed_model)), aes(x = residuals(mixed_model))) +
  ...
  theme_minimal()
# Histogram of residuals
#ggplot(data = as.data.frame(residuals(mixed_model_random)), aes(x = residuals(mixed_model_random))) +
#  geom_histogram(bins = 30) +
#  labs(title = "Residuals Distribution", x = "Residuals", y = "Frequency") +
#  theme_minimal()
...
#print(fixed_effects)
#fixed_effects <- broom.mixed::tidy(mixed_model_random)
#print(fixed_effects)
...
#random_effects <- VarCorr(mixed_model_random)
#print(random_effects)
